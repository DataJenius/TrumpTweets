...
<div class="byline">Published by Josh Pause @[DataJenius](https://datajenius.com) on April 7th, 2019</div>
<!-- <img class="mainpic" src="/articles/sentiment-analysis-with-trump-tweets/img1_trump.jpg"> -->
<div class="caption">Pictured above: the President hard at work</div>

## Introduction

In this article we will be using R, including the tidytext and sentimentr packages, to do some natural language processing and sentiment analysis of Trump's tweets. [All of the code and data used in the creation of this article is available via our Github depository](https://github.com/DataJenius/TrumpTweets).
...
The gross majority of these tweets (more than 98%) were posted using "Twitter for iPhone". This is not to say all of these tweets are actually written by Trump himself. It is quite possible there are social media or public relations staff handling some of these posts, but if they are, they are also using iPhones most of the time. 

There is an incredible amount of variance in the amount of retweets a post by Trump receives. The least popular tweet in our data got a mere 6 retweets: 
...
We don't have time to read through all of these tweets manually, but this is where the real "meat and potatoes" of the data is hiding. How can we use natural language processing, and more specifically, sentiment analysis, to dig deeper?
...
We find that roughly 82% of Trump's tweets contain at least one word matching "trust", followed by 63% which are associated with "anticipation", followed by 57% which are associated with "fear". Only 40% of Trump's tweet contain at least one word matching to "disgust". Remember: this can be deceptive because the same word (e.g. "deal") can be associated to multiple emotional categories. 
...
If we ignore the long tails (the tweets with extremely negative or positive sentiment), we find an approximately normal distribution, centered around a net sentiment near 5. 
...
Now that we have classified Trump's tweets into "positive" and "negative" sentiment, we can take a closer look, and see where this sentiment is coming from. 
...
If we break these tweets into quartiles based on net sentiment, we see a clear distinction between the 1st quartile (most negative) and 4th quartile (most positive). Let us count the most common bigrams appearing in both groups. In other words, which words are found most often among the most positive, and most negative, Trump tweets?
...
We can use the NRC lexicon to look at the specific emotions of these tweets about Mueller. 
...
Among this subset we find 86% of tweets correspond to "anger" and "fear", 81% correspond to "disgust", and 75% to "sadness", with very little "joy" or "surprise". This passes a sanity check.
...
Instead of subsetting tweets based on net sentiment, or a specific keyword, we might focus on a specific emotion from the NRC lexicon. For example: fear.
...
Which bigrams appear most commonly in the 4th Quartile? 

That is, what are the things Trump fears the most?
...
Until now, we have been determining sentiment on a word-by-word basis. The [sentimentr package](https://cran.r-project.org/web/packages/sentimentr/index.html) in R offers a different approach. Unlike our word-based lexicon methods, sentimentr considers valence shifters. Consider these two examples: 
...
Our lexicon method scored this tweet -48 (negative), while sentimentr gave it a score near 0 (neutral). Even a human reader may struggle with the sentiment of the above tweet, given the generous amount of sarcasm it contains. 
...
In order to better compare these two methods, we will standardize both sentiment scores into a z score, and then look at the difference between them. Under this standardized scale, a score of 0 corresponds to the average sentiment, negative numbers mean a tweet is more "negative" than average, and positive numbers mean a tweet is more "positive" than average. 

```{r echo=FALSE}
# standardize to z score
tweets.compare$lexicon <- (tweets.compare$sentiment-mean(tweets.compare$sentiment, na.rm=TRUE))/sd(tweets.compare$sentiment, na.rm=TRUE) 
tweets.compare$sentimentr <- (tweets.compare$sentimentr-mean(tweets.compare$sentimentr))/sd(tweets.compare$sentimentr)   
tweets.compare$diff <- tweets.compare$sentimentr-tweets.compare$lexicon
kable(head(subset(tweets.compare, select=c(id_str, sentimentr, lexicon, diff))))
```
...
Although the value of including valence shifters may not be immediately obvious in the above examples, looking at the big picture, we can usually expect a more reasonable sentiment score by considering them. 
...
sample$sentimentr <- last.example$ave_sentiment
tmp <- sample %>%
  unnest_tokens(word, text) %>%
  inner_join(lexicon) %>%
  group_by(id) %>%
  summarize(sentiment_score=sum(sentiment, na.rm=TRUE))
sample$lexicon <- tmp$sentiment_score
```
```{r echo=FALSE}
kable(sample)
```

Our lexicon method fails to see the difference between these sentences; sentimentr uses valence shifters to understand context in a way that no word-based method ever will.


## Conclusion 

As with all forms of unsupervised learning, when it comes to using natural language processing to extract sentiment, there is no official "right answer". Ultimately it comes down to the judgement and experience of the people assigned to the project. To any project manager, or researcher trying to replicate results, this can be equal parts frustrating and exhilarating.

There is a lot of fun to be had at the border of theory and application. Sentiment analysis is a critical skill for anyone working with NLP. The tidytext package in R, and sentimentr package, make it super easy to get a working prototype together in a few minutes. [Help yourself to our code](https://github.com/DataJenius/TrumpTweets), and have fun psychoanalyzing the President's tweets for yourself. 
